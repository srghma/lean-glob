...
def NonEmptyList.shrinkByRemovingIndividualElements (xs : NonEmptyList a) : List (NonEmptyList a) := (List.range xs.length).filterMap (λ i => NonEmptyList.fromList? $ xs.toList.eraseIdx i)
...
def NonEmptyList.shrinkByRemovingSuffixes (xs : NonEmptyList a) : List (NonEmptyList a) :=
  (List.range xs.length).reverse.filterMap fun n =>
    NonEmptyList.fromList? (xs.toList.take n)
